...
import { Leaf, Eye, EyeOff, Loader2, AlertTriangle, CheckCircle2 } from "lucide-react";
...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { error: err, needsVerification } = await signUpWithEmail(email, password, name);
    ...
    <div className="flex min-h-[80vh] items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        <div className="mb-8 text-center">
          ...
          <h1 className="mt-6 font-heading text-3xl font-bold text-foreground">Create an account</h1>
          <p className="mt-2 text-foreground/50">Start verifying real-world claims today</p>
        </div>
        ...
        <div className="rounded-card border border-border bg-surface p-8 shadow-sm">
          ...
          <form onSubmit={handleSignUp} className="space-y-5">
            <div>
              <label htmlFor="name" className="mb-1.5 block text-sm font-semibold text-foreground/70">Full Name</label>
              <input
                id="name"
                type="text"
                required
            ...
            <div>
              <label htmlFor="password" className="mb-1.5 block text-sm font-semibold text-foreground/70">Password</label>
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-btn border border-border bg-background px-4 py-3 pr-12 text-foreground placeholder:text-foreground/25 transition-colors focus:border-accent/50 focus:outline-none focus:ring-2 focus:ring-ring"
                />
                <button
                  type="button"
          ...
          <div className="my-6 flex items-center gap-3">
            ...
            <span className="text-xs font-medium uppercase tracking-wider text-foreground/35">or</span>
            <hr className="flex-1 border-border" />
          </div>
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-3 rounded-btn border border-border bg-background px-6 py-3 text-base font-semibold text-foreground shadow-sm transition-all duration-200 hover:bg-muted hover:border-foreground/20 cursor-pointer"
          >
        ...
        <p className="mt-6 text-center text-sm text-foreground/50">
          ...
          <Link to="/login" className="font-semibold text-accent hover:text-accent/80 transition-colors cursor-pointer">
            Sign In
          </Link>
        </p>
      </div>
